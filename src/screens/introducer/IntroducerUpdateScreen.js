...
const IntroducerUpdateScreen = function ({ navigation }) {
  const introducer = navigation.state.params;

  const [_date, _month, _year] = introducer.dateOfBirth
    ? introducer.dateOfBirth.split("/")
    : convertDateToString(new Date()).split("/");
  const [dob, setDob] = useState(new Date(+_year, +_month - 1, +_date));
  const [fullName, setFullName] = useState(introducer.fullName);
  const [gender, setGender] = useState(introducer.gender);
  const [dateOfBirth, setDateOfBirth] = useState(introducer.dateOfBirth);
  const [address, setAddress] = useState(introducer.address);
  const [email, setEmail] = useState(introducer.email);
  const [phone, setPhone] = useState(introducer.phone);
  const [identification, setIdentification] = useState(
    introducer.identification
  );
  const [image, setImage] = useState(introducer.image);

  const update = async function () {
    const id = await AsyncStorage.getItem("introducerId");
...
export default IntroducerUpdateScreen;
